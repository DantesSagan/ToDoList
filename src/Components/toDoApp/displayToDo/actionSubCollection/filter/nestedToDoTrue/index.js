...
export default function NestedSubObjTrue({
  disNameArray,
  nestedToDoArray,
  user,
  arrayID,
    ...
    return Object.keys(disNameArray[item]).map((ind) => {
      ...
      return Object.keys(nestedToDoArray).map((itemsNested) => {
        ...
        return Object.keys(nestedToDoArray[itemsNested]).map((index) => {
          ...
          // Check for displaying list of todos with doneToDo strict equal to true
          let equalToTrue =
          ...
          return (
            <div
              ...
              key={itemsNested.id}
            >
              {/* with check especially toDoId pathname and username and strict equal to doneToDo === true*/}
              {equalToTrue ? (
                <section>
                  {checkName &&
                  checkNestedID &&
