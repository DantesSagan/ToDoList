...
export default function HandleSubmitSubToDo({
  ...
  const handleSubmitSubToDo = async () => {
    ...
    return Object.keys(disNameArray).map(async (item) => {
      ...
      let getRouterPathToDo = `/todolist/${disNameArray[item].toDosArray.toDoID}`;
      ...
      if (checkPathIDToDoList) {
        window.confirm(
          `Are you sure you want to add this toDo = ${disNameArray[item].toDosArray.toDo}? Вы уверены, что хотите добавить дополнительный список дел ${disNameArray[item].toDosArray.title}?`
        );
      } else {
            ...
            return checkUserName && checkDockIDToDo
              ? await setDoc(nestedRef, {
                  toDosArray: {
                    ...
                    userId: userAuth,
                    toDoID: toDoID,
                    parentID: disNameArray[item].toDosArray.toDoID,
                    untilTime: untilTime,
                    doneToDo: false,
                    importance: ['white'],
                  },
